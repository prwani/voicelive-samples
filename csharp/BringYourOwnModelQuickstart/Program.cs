        ...
        private async Task ProcessAudioSendAsync(CancellationToken cancellationToken)
        {
            try
            {
                await foreach (byte[] audioData in _audioSendReader.ReadAllAsync(cancellationToken).ConfigureAwait(false))
                {
                    ...
                    catch (System.Net.WebSockets.WebSocketException ex) when (ex.WebSocketErrorCode == System.Net.WebSockets.WebSocketError.InvalidState)
                    {
                        // WebSocket is closed/aborted during shutdown - this is expected
                        _logger.LogInformation("WebSocket closed during shutdown, stopping audio send");
                        break;
                    }
                    catch (Exception ex)
                    {
